...
## 更新日志
- 2026-05-02 自建 90DNS conntest 服务器；hosts 模板换成 nh-server 权威列表 + `MY_90DNS_IP` 变量驱动；移除 OC_Toolkit 与 Sphaira；sysMMC 补全遥测屏蔽；修复 hekate `kip1patch=nosigchk` 兼容性报错；`DEBUG=1` 时屏蔽 `GITHUB_TOKEN` 泄露
